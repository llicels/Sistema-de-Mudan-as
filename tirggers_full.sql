...
CREATE OR REPLACE FUNCTION calculate_service_price(p_cod_pedido INTEGER, p_nome_servico VARCHAR)
RETURNS NUMERIC AS $$
DECLARE
...
BEGIN
    -- Preço/hora ofertado pela empresa na cidade de partida
    ...
    SELECT tempo_duracao INTO v_tempo_duracao
...
DECLARE
    v_preco NUMERIC;
BEGIN
    v_preco := calculate_service_price(NEW.cod_pedido, NEW.nome_servico);
...
CREATE OR REPLACE FUNCTION update_solicitacao_prices(p_nome_servico VARCHAR, p_empresa INTEGER DEFAULT NULL, p_cidade VARCHAR DEFAULT NULL)
RETURNS VOID AS $$
BEGIN
  
    UPDATE solicitacoes_servico ss
    SET preco = COALESCE(calculate_service_price(ss.cod_pedido, ss.nome_servico), ss.preco)
    FROM pedidos p
...
-- oferecem: quando preco_hora muda
...
-- guindastes: quando bonus_aumento muda
...
-- acrescimos_transporte: qualquer mudança nas faixas
